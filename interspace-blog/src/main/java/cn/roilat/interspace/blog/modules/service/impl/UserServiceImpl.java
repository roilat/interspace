...
import cn.roilat.interspace.blog.base.lang.EntityStatus;
...
import cn.roilat.interspace.blog.base.utils.MD5;
import cn.roilat.interspace.blog.modules.data.AccountProfile;
import cn.roilat.interspace.blog.modules.data.BadgesCount;
import cn.roilat.interspace.blog.modules.data.UserVO;
import cn.roilat.interspace.blog.modules.entity.User;
import cn.roilat.interspace.blog.modules.repository.UserRepository;
...
//		Assert.state(po.getStatus() != Const.STATUS_CLOSED, "您的账户已被封禁");
	...
	public AccountProfile updateEmail(long id, String email) {
		...
		if (email.equals(po.getEmail())) {
			throw new LinjiezhijiaBlogException("邮箱地址没做更改");
		}

		User check = userRepository.findByEmail(email);

		if (check != null && check.getId() != po.getId()) {
			throw new LinjiezhijiaBlogException("该邮箱地址已经被使用了");
		}
		po.setEmail(email);
		userRepository.save(po);
		return BeanMapUtils.copyPassport(po);
