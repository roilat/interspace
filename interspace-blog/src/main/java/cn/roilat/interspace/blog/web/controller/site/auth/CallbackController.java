...
import cn.roilat.interspace.blog.base.lang.LinjiezhijiaBlogException;
import cn.roilat.interspace.blog.base.lang.Result;
import cn.roilat.interspace.blog.base.oauth.*;
import cn.roilat.interspace.blog.base.oauth.utils.OpenOauthBean;
import cn.roilat.interspace.blog.base.oauth.utils.TokenUtil;
import cn.roilat.interspace.blog.base.utils.FilePathUtils;
import cn.roilat.interspace.blog.base.utils.ImageUtils;
...
import cn.roilat.interspace.blog.modules.service.OpenOauthService;
...
public class CallbackController extends BaseController {
	...
	public void callWeibo(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			log.error("跳转到微博授权接口时发生异常");
			throw new LinjiezhijiaBlogException("跳转到微博授权接口时发生异常");
		}
	}
	...
	public String callback4Weibo(String code, String state, HttpServletRequest request, ModelMap model)
			throws Exception {
		// --
		String session_state = (String) request.getSession().getAttribute(SESSION_STATE);
		// 取消了授权
		if (StringUtils.isBlank(state) || StringUtils.isBlank(session_state) || !state.equals(session_state)
				|| StringUtils.isBlank(code)) {
			log.error("缺少必要的参数");
			throw new LinjiezhijiaBlogException("缺少必要的参数");
		}
	...
	public void callQQ(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			log.error("跳转到QQ授权接口时发生异常");
			throw new LinjiezhijiaBlogException("跳转到QQ授权接口时发生异常");
		}
	}
	...
	public String callback4QQ(String code, String state, HttpServletRequest request, ModelMap model) {
		...
		try {
			...
			log.error("解析信息时发生错误");
			throw new LinjiezhijiaBlogException("解析信息时发生错误");
		}
	...
	public void callDouban(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			log.error("跳转到豆瓣授权接口时发生异常");
			throw new LinjiezhijiaBlogException("跳转到豆瓣授权接口时发生异常");
		}
	}
	...
	private String login(String username, String accessToken) {
		...
		log.error("登录失败");
		throw new LinjiezhijiaBlogException("登录失败");
	}
