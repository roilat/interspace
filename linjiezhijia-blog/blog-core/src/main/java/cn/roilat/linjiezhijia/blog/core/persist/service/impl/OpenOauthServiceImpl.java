...
import cn.roilat.linjiezhijia.blog.core.persist.dao.OpenOauthDao;
import cn.roilat.linjiezhijia.blog.core.persist.dao.UserDao;
import cn.roilat.linjiezhijia.blog.core.persist.entity.OpenOauthPO;
...
public class OpenOauthServiceImpl implements OpenOauthService {
    ...
    public User getUserByOauthToken(String oauth_token) {
        ...
        UserPO userPO = userDao.getOne(thirdToken.getId());
    ...
    public boolean checkIsOriginalPassword(long userId) {
        OpenOauthPO po = openOauthDao.findByUserId(userId);
        if (po != null) {
            UserPO upo = userDao.getOne(userId);
